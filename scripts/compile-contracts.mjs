...
fs.mkdirSync("contract-artifacts",{recursive:true});for(const[file,contracts]of Object.entries(output.contracts)){for(const[name,artifact]of Object.entries(contracts)){const base=`${file.replace(/[\\/.]/g,"_")}_${name}`;fs.writeFileSync(path.join("contract-artifacts",`${base}.abi`),JSON.stringify(artifact.abi));fs.writeFileSync(path.join("contract-artifacts",`${base}.bin`),artifact.evm.bytecode.object)}}
