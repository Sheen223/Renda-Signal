...
export async function POST(request:Request){
  ...
  const needsChain=signal.status==="accepted";if(!needsChain)await runtime.DB.prepare("UPDATE signals SET status='submitted',updated_at=? WHERE id=?").bind(now,signalId).run();
