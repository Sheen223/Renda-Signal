...
export async function POST(request:Request){
  ...
  const signal=await runtime.DB.prepare("SELECT s.employee_wallet,s.status,COALESCE(p.payment_mode,'polygon') payment_mode FROM signals s LEFT JOIN signal_payment_profiles p ON p.signal_id=s.id WHERE s.id=? AND (s.target_x_id=? OR (s.target_x_id='' AND lower(s.target_handle)=lower(?)))").bind(signalId,profile.id,`@${profile.username}`).first<Record<string,string>>();
