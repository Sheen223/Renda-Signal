...
export async function POST(request:Request){
  ...
  const signal=await runtime.DB.prepare("SELECT employee_wallet,status FROM signals WHERE id=? AND lower(target_handle)=lower(?)").bind(signalId,`@${profile.username}`).first<Record<string,string>>();
  if(!signal||signal.status!=="accepted"||signal.employee_wallet?.toLowerCase()!==wallet)return Response.json({error:"Only the accepted recipient wallet can submit this work."},{status:403});
  ...
  return Response.json({id,contentHash:digest,fileCount:files.length,linkCount:links.length},{status:201});
}
