...
export async function POST(request:Request){
  ...
  const signal=await runtime.DB.prepare("SELECT * FROM signals WHERE id=? AND lower(target_handle)=lower(?) AND status='funded'").bind(body.id,`@${profile.username}`).first<Record<string,string|number>>();
  ...
  const expiry=BigInt(Math.floor(Date.now()/1000)+600),nonce=keccak256(stringToHex(crypto.randomUUID())),contractId=BigInt(String(signal.contract_request_id)),targetIdentity=keccak256(stringToHex(`x-handle:${String(signal.target_handle).toLowerCase()}`));
