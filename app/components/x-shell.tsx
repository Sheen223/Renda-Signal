...
export function XShell({children}:{children:ReactNode}){
  ...
  const[nimAdmin,setNimAdmin]=useState(false);
  ...
  useEffect(()=>{
    ...
    fetch("/api/signals/nimiq?checkAdmin=1").then(r=>r.ok?r.json():null).then(data=>setNimAdmin(Boolean(data?.isAdmin))).catch(()=>setNimAdmin(false));
  },[]);
  ...
  const links=[["/x","For me"],["/x/sent","Sent"],["/x/history","History"],["/x/new","New signal"],...(nimAdmin?[["/x/nim-admin","NIM Admin"]]:[]),...(wallet&&wallet.toLowerCase()===arbitrator.toLowerCase()?[['/x/arbitration','Admin']]:[])];
